...
const Registration_precheck = (req, res, next) => {
  ...
  let msg = "All required fields should be present: name, email, mobile & Password";
  let status = 0;
  let data = [];
...
const check_login_or_not_user = async(req,res,next) =>{
  ...
  try {
      ...
      req.user = {user_id:decoded.decode.u_id}
      const isMatch =await UserModel.findOne({_id:req.user.user_id});
      if(!isMatch){
