  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      if (response.data.status === 1) {
        setCookie('reg_token',response.data.token)
        setOtpBox(true);
        setError('');
      } else {
    ...
    <div className="flex items-center justify-center px-4 py-10 sm:px-6 sm:py-16 lg:px-8 lg:py-24">
      <BackgroundBackdrop/>
        <div className=" xl:mx-auto xl:w-full xl:max-w-sm w-full md:mx-auto md:w-1/2 2xl:max-w-md">
          ...
          {error && <div className="rounded-md mb-5 border-l-4 border-violet-500 bg-violet-100 p-4">
          ...
          {OTPerror && <div className="rounded-md mb-5 border-l-4 border-violet-500 bg-violet-100 p-4">
            <div className="flex items-center space-x-4">
            <div>
                <AlertCircle className="h-6 w-6 text-violet-600" />
            </div>
